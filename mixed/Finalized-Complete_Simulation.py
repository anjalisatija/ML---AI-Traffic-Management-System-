...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        path = "D:\\KUSHAGRA\\DATA\\K_Works\\ML AI TMS\\mixed\\images\\" + direction + "\\" + vehicleClass + ".png"
...
def printStatus():                                                                                           
	for i in range(0, noOfSignals):
		if(i==currentGreen):
			if(currentYellow==0):
				print("GREEN Traffic Signal",i+1,"-> red:",signals[i].red," yellow:",signals[i].yellow," green:",signals[i].green)
			else:
				print("YELLOW Traffic Signal",i+1,"-> red:",signals[i].red," yellow:",signals[i].yellow," green:",signals[i].green)
		else:
			print("RED Traffic Signal",i+1,"-> red:",signals[i].red," yellow:",signals[i].yellow," green:",signals[i].green)
	print()
...
def simulationTime():
    ...
    while(True):
        ...
        if(RoadRage==simTime):
            ...
            print('Total time passed: ',RoadRage)
            print('No. of vehicles passed per unit time: ',(float(totalVehicles)/float(RoadRage)))
            os._exit(1)
...
class RNNModel:
    def __init__(self):
        ...
        self.biases = [np.random.rand(layer) for layer in [12, 10, 9, 8, 10, 8]]
...
def update_traffic_lights(signal_states, vehicle_data):
    global RoadRage

    vehicle_data_flat = []
    for vehicle in vehicle_data:
        vehicle_data_flat.append(vehicle[-1])  
    vehicle_data_flat = vehicle_data_flat + signal_states 

   
    input_features = np.array(vehicle_data_flat)


    if len(input_features) != 20:
        return
    
    output = rnn_model.forward(input_features)

    for i in range(len(signal_states)):
        if output[i] > 0.5:
            signal_states[i] = 1  
        else:
            signal_states[i] = 0  
...
class Main:
    ...
    background = pygame.image.load('D:\\KUSHAGRA\\DATA\\K_Works\\ML AI TMS\\mixed\\images\\mod_int.png')

    screen = pygame.display.set_mode(screenSize)
    pygame.display.set_caption("ML AI-TMS | NEAT RNN Simulation")

    redSignal = pygame.image.load('D:\\KUSHAGRA\\DATA\\K_Works\\ML AI TMS\\mixed\\images\\signals\\red.png')
    yellowSignal = pygame.image.load('D:\\KUSHAGRA\\DATA\\K_Works\\ML AI TMS\\mixed\\images\\signals\\yellow.png')
    greenSignal = pygame.image.load('D:\\KUSHAGRA\\DATA\\K_Works\\ML AI TMS\\mixed\\images\\signals\\green.png')
    ...
    while True:
        ...
        RoadRageText = font.render(("Time Elapsed:"+str(RoadRage)+",RNN Gen.: 300"), True, black, white)

        screen.blit(RoadRageText,(1100,50))

        update_traffic_lights(signal_states, vehicles)
